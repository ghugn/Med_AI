"""
RAG Service — MedPilot
Singleton adapter của MedPilotRAGModule cho FastAPI async.
Load Excel → ChromaDB → query → vLLM.
"""
import asyncio
import time
...
# Lazy import của RAG module (nặng, chỉ load khi cần)
_rag_module = None
...
def _get_rag_module():
    """Lazy-load MedPilotRAGModule. Import ở runtime để tránh crash khi thiếu package."""
    try:
        # Import trực tiếp class từ file gốc ở root project
        import sys
        import os
        root = Path(__file__).parent.parent.parent  # backend_AI/
        if str(root) not in sys.path:
            sys.path.insert(0, str(root))
        from medpilot_rag_module_Version1 import MedPilotRAGModule
...
def initialize() -> bool:
    """
    Khởi tạo RAG module (đồng bộ, gọi từ startup event của FastAPI).
    - Load embeddings model
    - Kết nối ChromaDB
    - Load + index Excel nếu thư mục tồn tại và có file

    Returns:
        True nếu khởi tạo thành công, False nếu lỗi hoặc không có data
    """
    global _rag_module, _rag_initialized, _rag_error

    excel_folder = Path(settings.RAG_EXCEL_FOLDER)
    ...
    try:
        ...
        _rag_module = MedPilotRAGModule(
            ...
            cache_path=str(settings.RAG_CACHE_PATH),
            # Truyền dummy URL vì ta override query bằng vllm_client
            chat_api_url=f"{settings.VLLM_BASE_URL}/chat/completions",
...
def get_status() -> Dict:
    ...
    status = {
        ...
        "error": _rag_error,
        "excel_folder": str(settings.RAG_EXCEL_FOLDER),
        "db_path": str(settings.RAG_DB_PATH),
    }
    if is_ready():
        try:
...
async def query(question: str, top_k: Optional[int] = None) -> Dict:
    ...
    Args:
        ...
        top_k: Số chunks lấy làm context (None = dùng config)

    Returns:
        {"query", "answer", "sources", "latency", "context_chunks"}
    """
    start = time.time()
    ...
    # 1. Retrieve context từ ChromaDB (chạy trong thread để không block event loop)
    ...
    if not retrieved:
        ...
        sources = list(dict.fromkeys(r["disease"] for r in retrieved))  # unique, preserve order
    ...
    # 3. Gọi vLLM
    if context:
        answer = await vllm_client.chat_completion_safe(
            messages=messages,
            fallback=(
        ...
        # Không có context → trả lời dựa trên kiến thức general + safety disclaimer
