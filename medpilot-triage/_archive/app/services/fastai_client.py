"""
FastAI Client — MedPilot Comprehensive Triage
Gọi vLLM thông qua vllm_client, với fallback mock khi vLLM chưa sẵn sàng.
"""
import time
import json
import asyncio
...
from app.models.schemas import (
    ClinicalState, GateDecision, QuestionItem, AIResponseData
)
...
async def extract_and_triage(
    ...
    """Trích xuất thông tin lâm sàng và đánh giá triage. Gọi vLLM, fallback mock."""
    logger.info(f"[FastAI] extract_and_triage — case {case_id}")
    ...
    raw = await vllm_client.chat_completion_safe(messages=messages, max_tokens=2048, temperature=0.1)

    try:
        data = _parse_json_response(raw)
        return AIResponseData(**data)
    except Exception as e:
        logger.warning(f"[FastAI] Parse lỗi triage response: {e}. Dùng fallback mock.")
...
async def review_clinical_summary(
    ...
    logger.info(f"[FastAI] review_clinical_summary — case {case_id}")
    ...
    raw = await vllm_client.chat_completion_safe(messages=messages, max_tokens=1024, temperature=0.1)

    try:
        data = _parse_json_response(raw)
        return AIResponseData(**data)
    except Exception as e:
        logger.warning(f"[FastAI] Parse lỗi review response: {e}. Dùng fallback mock.")
        return AIResponseData(
            state=ClinicalState(
                entities={},
...
async def evaluate_workflow_decision(
    ...
    """Đánh giá bước tiếp theo của workflow (PASS/NEED_INPUT/NEED_REVIEW)."""
    logger.info(f"[FastAI] evaluate_workflow_decision — case {case_id}")
    ...
    raw = await vllm_client.chat_completion_safe(messages=messages, max_tokens=512, temperature=0.1)

    try:
        data = _parse_json_response(raw)
        return AIResponseData(**data)
    except Exception as e:
        logger.warning(f"[FastAI] Parse lỗi workflow response: {e}. Dùng fallback mock.")
...
async def merge_question_answers(
    ...
    logger.info(f"[FastAI] merge_question_answers — case {case_id}")
    ...
    try:
        ...
        logger.warning(f"[FastAI] Parse lỗi merge response: {e}. Dùng fallback mock.")
        # Fallback mock
...
async def chat_interaction(user_role: str, message: str, history: list) -> str:
    ...
    logger.info(f"[FastAI] chat_interaction — role={user_role}")
    ...
    return await vllm_client.chat_completion_safe(
        messages=messages,
        fallback=fallback,
        max_tokens=512,
        temperature=0.7,
